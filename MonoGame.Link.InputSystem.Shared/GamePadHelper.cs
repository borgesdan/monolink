...
namespace Microsoft.Xna.Framework.InputSystem
{
    ...
    public class GamePadHelper : IUpdate
    {
        ...
        Dictionary<Buttons, Keys?> keyboardMap = new Dictionary<Buttons, Keys?>();
        ...
        /// <param name="map">O mapeamento de teclas para botões do GamePad.</param>
        /// <param name="padState">Define o estdo do GamePad.</param>
        public GamePadHelper(PlayerIndex playerIndex, KeyButtonMap map = null, GamePadState? padState = null)
        {
            Index = playerIndex;

            if(padState.HasValue)
                State = padState.Value;
        ...
        private bool CheckButton(Buttons button, Predicate<Keys> function)
        {
            ...
            if (k.HasValue)
            {
                return function.Invoke(k.Value);
            }                
            else
                return false;
        }
